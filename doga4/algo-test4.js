...
function removeFirstAndLast(string) {
    let finaltext="";
    finaltext=string.split(",").slice(1,-1).join(" ") || null
    return finaltext;
}
...
function dnaStrand(dna){
    let dnaText="";
    let dnaKeys={"A":"T","T":"A","C":"G","G":"C"};
    for (let i = 0; i < dna.length; i++) {
        let char = dna[i];
        dnaText+=dnaKeys[char];  
    }
    return dnaText;
}
...
function deEmojify(emojiString) {
    ...
    let eredmeny='';
    if (emojiString) {
        let tomb=emojiString.split('  ')
        for(let element of tomb){
            let a=element.split(' ').map(elem => jelek.indexOf(elem)).join('')
            eredmeny += String.fromCharCode(parseInt(a))
        }
    }
    return eredmeny;
}
